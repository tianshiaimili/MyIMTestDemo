...
public class SmackImpl implements Smack {
	public static final String XMPP_IDENTITY_NAME = Constants.XMPP_IDENTITY_NAME;
	public static final String XMPP_IDENTITY_TYPE = Constants.XMPP_IDENTITY_TYPE;
	...
	public SmackImpl(ActionService service) {
	...
	public boolean login(String account, String password) throws IMException {
		...
		try {
			...
			if (!mXMPPConnection.isConnected()) {
				throw new IMException("SMACK connect failed without exception!");
			}
...
//				mXMPPConnection.login(account, password, ressource);
			...
			throw new IMException(e.getLocalizedMessage(),
			...
			LogUtils2.e("error  == "+e.getMessage());
			throw new IMException(e.getLocalizedMessage(), e.getCause());
		}
	...
	private class PongTimeoutAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			...
			mService.postConnectionFailed(ActionService.PONG_TIMEOUT);
			logout();// 超时就断开连接
		}
	}
	...
	private void removeRosterEntry(String user) throws IMException {
		mRoster = mXMPPConnection.getRoster();
		try {
	...
	public void renameRosterItem(String user, String newName)
		...
		if (!(newName.length() > 0) || (rosterEntry == null)) {
			throw new IMException("JabberID to rename is invalid!");
		}
	...
	private void tryToRemoveUserFromGroup(RosterGroup group,
			RosterEntry rosterEntry) throws IMException {
		try {
